...
apt-get install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" net-tools unzip git make build-essential libtool automake autotools-dev autoconf pkg-config libssl-dev libgmp-dev  libevent-dev libdb4.8-dev libdb4.8++-dev libminiupnpc-dev libboost-all-dev ufw fail2ban pwgen curl>/dev/null 2>&1
...
if [ "$?" -gt "0" ];
  then
    ...
    echo "apt-get update"
    echo "apt install -y net-tools unzip  git make build-essential libtool automake autotools-dev autoconf pkg-config libssl-dev libgmp-dev libevent-dev libdb4.8-dev libdb4.8++-dev libminiupnpc-dev libboost-all-dev"
    exit 1
fi
clear
